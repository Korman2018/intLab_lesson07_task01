...
public class YandexMapsCoordinatesRequester {
    ...
    public static String getCoordinates(String place) {
        ...
        String bodyOne = Objects.requireNonNull(responseOne).getBody();
        String uid = findYandexUID(responseOne);
        String csrfToken = findToken(bodyOne);

        LOGGER.debug("{} from response header", uid);
        LOGGER.debug("csrfToken={} from response body", csrfToken);

        LOGGER.debug("GET request two");
        ...
        String bodyTwo = Objects.requireNonNull(responseTwo).getBody();
        String coordinateOne = findCoordinate(bodyTwo, 1);
        String coordinateTwo = findCoordinate(bodyTwo, 2);

        LOGGER.info("Coordinates for place:'{}' (latitude = {} longitude = {})", place, coordinateTwo, coordinateOne);
        return coordinateTwo + " " + coordinateOne;
    }

    private static String findCoordinate(String source, int numberOfCoordinate) {
        return findFirstFindSubstringByRegex(source, COORDINATES_REGEX, numberOfCoordinate);
    }

    private static String findYandexUID(HttpResponse<String> response) {
        String headerWithYandexUID = response.getHeaders().getFirst(HEADER_WITH_YANDEX_UID);
        return findFirstFindSubstringByRegex(headerWithYandexUID, YANDEX_UID_REGEX, 0);
    }

    private static String findToken(String source) {
        return findFirstFindSubstringByRegex(source, CSRF_TOKEN_REGEX, 1);
